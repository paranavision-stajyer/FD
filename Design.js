...
//parcaListesi[i].geometry.faces[j].materialIndex=6;
//parcaListesi[i].geometry.elementsNeedUpdate=true;
...
function onDocumentMouseDown(event) {
    ...
    switch (baslaTiklandi) {
        ...
        case 2://ikinci panel için kırmızı boyama işlemi
            ...
            for (i = 0; i < parcaListesi.length; i++) {
                if (kesisen[0].object == parcaListesi[i]) {
                    panel2 = i;
                    if (panel1 == panel2) {
                        ...
                        if (parcaAdi == paneller[q]) {
                            for (p = 0; p < 12; p++) {
                                if (parcaListesi[q].geometry.faces[p].color.r != 0 && parcaListesi[q].geometry.faces[p].color.g != 0 && parcaListesi[q].geometry.faces[p].color.b != 0) {
                                    ...
                                    parcaListesi[q].geometry.faces[p].color.setHex(0xffffff);
                                }
...
function loadAssy(str) {
    ...
    for (i = 0; i < x.length; i++) {
        if (x[i].nodeName == "panel") {
            ...
            var material0 = createLabel("2", 50);
            var material1 = createLabel("3", 50);
            var material2 = createLabel("4", 50);
            var material3 = createLabel("5", 50);
            var material4 = createLabel("0", 50);
            var material5 = createLabel("1", 50);
    ...
    new THREE.MeshPhongMaterial({
        color: 0x0000ff
            ...
            /*var texture1 = r.load( "textures/0.jpeg" );
            ...
            while (1) {
                ...
                /*for(j=0;j<parcaListesi.length;j++){
            for(i=0;i<12;i++){
            parcaListesi[j].geometry.faces[i].materialIndex=6;}}*/
                a = a + 1;
            }
